...
OMICSER_RUN_DIR <- getwd() # file.path(REPO_PATH,"quickstart")

# where are the raw data files for this script?
RAW_DATA_DIR <- file.path(OMICSER_RUN_DIR, "examples/raw_data/rik_data")
...
DB_NAME <-  list("Rik lipids" = "rik_lipids")
...
prep_lipidizer_files <- function(data_file, path_root){
  ...
  # create the matrix
  dat_mat <- raw_table[, !..meta_cols]
  dat_mat <- as.matrix(dat_mat)
  rownames(dat_mat) <- obs_meta$sample_id
...
RAW_DIR <- file.path(OMICSER_RUN_DIR, "examples", "raw_data", "rik_data")

# a. load concentration data --------------------

conc_csv_name <- "NB_data_20211209_y.csv"
...
config_list <- list(
  ...
  group_obs = c("Genotype", "Cell_number_k"),
